words={'begin':1,'end':2,'integer':3,'char':4,'bool':5,'real':6,'input':7,'output':8,'program':9,'read':10,'write':11,
      'for':12,'to':13,'while':14,'do':15,'repeat':16,'until':17,'if':18,'then':19,'else':20,'true':43,'false':43,
      'var':23,'const':24,'+':25,'-':26,'*':27,'/':28,'=':29,'<':30,'>':31,'and':32,'or':33,'not':34,'<=':35,'>=':36,'<>':37,
        ...
        while pointer<text_len-1:
            ...
            if text[pointer].isalpha():
                ...
                if text[pointer].isdigit():
                    ...
                    pointer+=1
                    while pointer<text_len :
                        if  text[pointer].isdigit() or text[pointer]=='.' or text[pointer]=='E'or text[pointer]=='e' or text[pointer]=='+' or text[pointer]=='-':
                            str=str+text[pointer]
                            pointer+=1
                        else:
                            break
